...
const PaymentModes = () => {
    ...
    const updateModeSelection = (itemIndex) => {
        setModes((prev) => prev.map((item, index) => {
            ...
            <div className="w-[330px] sm:w-[500px]">
                ...
                <div className="max-h-[340px] overflow-y-scroll">
                    {modes.map((item, index) => (
                        <div className={`${item.isSelected ? 'border-black border-2' : 'border-slate-300 border'} p-2 rounded-lg mt-4 cursor-pointer`} onClick={() => updateModeSelection(index)}>
